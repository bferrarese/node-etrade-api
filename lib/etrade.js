...
var modules = [require('./authorization.js'),
               ...
               require('./market.js'),
               require('./order.js'),
               require('./limits.js')]; // List of E*TRADE modules
...
exports.prototype._run = function(actionDescriptor,params)
{
      if (!this.authorized)
          return Promise.reject("Please authorize first; Request: [" + requestOptions.method + "]: " + requestOptions.url + (qs.length ? "?" + qs : ""));
    ...
    this.request(requestOptions,function(error,message,body)
    {
        ...
        else if (message.statusCode != 200)
        {
            ...
            reject("E*TRADE responded with a non-OK HTTP status code: " + JSON.stringify(msg));
        }
        else
        {
